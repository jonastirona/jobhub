...
export default function DocumentLibrary() {
  ...
  // Display all versions of documents in the library.
  // Show all versions so users can see the complete version history.
  ...
  function expandAllDetails() {
    setExpandedDocIds(new Set(displayDocuments.map((doc) => doc.id)));
  }
  ...
  async function handleDuplicateAsVersion() {
    if (!versioningDoc) return;
    await duplicateDocument(versioningDoc.id);
    closeVersionModal();
  }

  async function handleUploadNewVersion(event) {
    const file = event.target.files?.[0];
    if (!versioningDoc || !file || !session?.access_token) return;
    ...
    formData.append('name', versioningDoc.name || file.name.replace(/\.[^.]+$/, '') || 'Document');
    formData.append('doc_type', versioningDoc.doc_type || 'Draft');
    formData.append('source_document_id', versioningDoc.id);
    if (versioningDoc.job_id) {
      formData.append('job_id', versioningDoc.job_id);
    }
    if (versioningDoc.status) {
      formData.append('status', versioningDoc.status);
    }
    if (Array.isArray(versioningDoc.tags) && versioningDoc.tags.length > 0) {
      formData.append('tags', JSON.stringify(versioningDoc.tags));
    }
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(`Failed to upload version (${res.status})`);
      }
      await refetch();
      closeVersionModal();
    } catch (err) {
      alert(`Error uploading version: ${err instanceof Error ? err.message : String(err)}`);
    }
  }

  // Check if a document has other versions in the version group
  function getVersionCount(doc) {
    const group = doc.version_group_id ?? doc.id;
    return documents.filter((d) => (d.version_group_id ?? d.id) === group).length;
  }

  // Get version badge text if document is part of a version group
  function getVersionBadge(doc) {
    const count = getVersionCount(doc);
    if (count > 1) {
    ...
    <AppShell title="Document Library" notificationCount={0}>
            ...
            <div className="dashboard-sort-control">
              ...
              <div className="document-details-controls">
                <button
                  ...
                  disabled={displayDocuments.length === 0}
                >
                  Expand all
                </button>
                <button
      ...
      {showVersionModal && versioningDoc && (
        ...
        >
          ...
          >
            <h2 className="document-view-modal-title" id="version-modal-title">
              Create New Version
            </h2>
            <p className="document-view-modal-text">
            ...
            <div style={{ display: 'flex', gap: 8, flexDirection: 'column' }}>
              <button
                type="button"
                className="document-view-modal-btn"
                onClick={handleDuplicateAsVersion}
